    ...
    for update in updates:
        ...
        if callback:
            ...
            if data.startswith("write_"):
                answer_callback_query(callback_id, "âœï¸ Processing request...")
...
def _handle_write_article(topic_hash=None):
    ...
    if _article_attempted_this_run:
        logger.info("â­ï¸ Skipping duplicate write_article â€” already attempted this run")
        send_simple_message("âœï¸ An article generation is already in progress or was just attempted. Please wait a moment.")
        return

    if _gemini_quota_exhausted:
        logger.info("â¸ï¸ Skipping article generation â€” Gemini quota exhausted this cycle")
        send_simple_message("â¸ï¸ Gemini API quota exhausted. Article generation paused until next cycle.")
        return
...
def _handle_approve(status="draft"):
    ...
    try:
        ...
        if result:
            ...
            send_simple_message("âŒ WordPress publishing failed. Check your WP credentials.")
    except Exception as e:
...
def test_all_connections():
    ...
    for name, url in list(config.RSS_FEEDS.items())[:3]:
        try:
            ...
            if feed.entries:
                ...
                print(f"   âš ï¸ {name}: No entries (feed may be empty or blocked)")
        except Exception as e:
            print(f"   âŒ {name}: {e}")
    ...
    try:
        ...
        if trending is not None and not trending.empty:
            print(f"   âœ… Connected â€” {len(trending)} trending searches found")
        else:
            print("   âš ï¸ Connected but no trending data returned")
    except Exception as e:
        print(f"   âŒ FAILED â€” {e}")

    # WordPress
    print("\n5ï¸âƒ£  WordPress REST API:")
    try:
        import requests
        resp = requests.get(
        ...
        if resp.status_code == 200:
            ...
            print(f"   âœ… Connected â€” Categories: {', '.join(cats[:5])}")
        else:
            print(f"   âŒ FAILED â€” HTTP {resp.status_code}")
    except Exception as e:
        print(f"   âŒ FAILED â€” {e}")
    ...
    try:
        ...
        print(f"   âœ… Connected â€” Response: {response.text.strip()}")
    except Exception as e:
        print(f"   âŒ FAILED â€” {e}")
