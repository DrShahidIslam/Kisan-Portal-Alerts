    ...
    for update in updates:
        ...
        if callback:
            ...
            if data.startswith("write_"):
                answer_callback_query(callback_id, "⌛ Checking topic...")
                ...
                if getattr(config, "REQUIRE_ARTICLE_CONFIRMATION", True):
                    ...
                    if topic:
                        ...
                        send_simple_message("⚠️ Could not find topic details for confirmation.")
                else:
                    _handle_write_article(topic_hash)

            elif data.startswith("confirm_write_"):
                answer_callback_query(callback_id, "✍️ Starting generation...")
                topic_hash = data.split("_")[-1]
                if topic_hash == "none": topic_hash = None
                _handle_write_article(topic_hash)

            elif data == "cancel_write":
                answer_callback_query(callback_id, "❌ Generation cancelled.")
                send_simple_message("❌ Article generation cancelled.")
...
def _handle_write_article(topic_hash=None):
    ...
    if _article_attempted_this_run:
        logger.info("🕙 Skipping duplicate write_article — already attempted this run")
        send_simple_message("✍️ An article generation is already in progress or was just attempted. Please wait a moment.")
        return

    if _gemini_quota_exhausted:
        logger.info("🔋 Skipping article generation — Gemini quota exhausted this cycle")
        send_simple_message("🔋 Gemini API quota exhausted. Article generation paused until next cycle.")
        return
    ...
    for name, url in list(config.RSS_FEEDS.items())[:3]:
        try:
            ...
            if feed.entries:
                ...
                print(f"   ⚠️ {name}: Feed parsed but empty")
        except Exception as e:
            print(f"   ❌ {name}: FAILED ({e})")

    # WordPress
    print("\n4️⃣  WordPress (REST API):")
    from publisher.wordpress_client import test_wp_connection
    ok, wp_info = test_wp_connection()
    if ok:
        print(f"   ✅ Connected to {wp_info['url']} as {wp_info['user']}")
    else:
        print(f"   ❌ FAILED — {wp_info}")

    # Gemini
    print("\n5️⃣  Gemini (Google Generative AI):")
    from gemini_client import test_gemini_connection
    ok, gem_info = test_gemini_connection()
    if ok:
        print(f"   ✅ Connected (Models available: {len(gem_info['models'])})")
    else:
        print(f"   ❌ FAILED — {gem_info}")
