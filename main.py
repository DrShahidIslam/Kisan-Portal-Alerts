"""
Kisan Portal Alerts Agent â€” Main Entry Point
...
def _is_scheme_topic(topic):
    """Return True if the topic clearly maps to a scheme-led article."""
    try:
        if not isinstance(topic, dict):
            return False
        if topic.get("scheme_id"):
            return True
        scheme = find_best_scheme(f"{topic.get('topic', '')} {topic.get('matched_keyword', '')}")
        return bool(scheme)
    except Exception:
        return False
...
def run_scan():
    ...
    try:
        ...
        min_total_scheme = getattr(config, "MIN_SCHEME_TOPICS_PER_CYCLE", 5)
        existing_scheme_topics = sum(1 for t in trending_topics if _is_scheme_topic(t))
        coverage_needed = max(
            0,
            max(
                getattr(config, "MIN_COVERAGE_TOPICS_PER_CYCLE", 4) - len(trending_topics),
                min_total_scheme - existing_scheme_topics,
            ),
        )
...
def _handle_write_article(topic_hash=None):
    ...
    try:
        ...
        if article:
            article["matched_keyword"] = article.get("focus_keyword") or topic.get("matched_keyword", "")  # For RankMath focus keyword
            ...
            quality = validate_article_for_publish(article, min_words=getattr(config, "ARTICLE_MIN_WORDS", 700))
            article["quality_snapshot"] = quality
            _pending_article = article
            send_article_preview(article, quality=quality)
...
def _handle_approve(status="draft", bypass_quality_gate=False):
    ...
    _pending_article["quality_snapshot"] = quality
...
def run_agent_loop():
    ...
    logger.info("ðŸ¤– Kisan Portal Alerts Agent starting up...")
    ...
    while True:
        try:
            ...
            send_simple_message("â¹ï¸ Kisan Portal Agent has been stopped.")
            break
        except Exception as e:
...
def test_all_connections():
    ...
    if ok:
        ...
        mid = send_simple_message("ðŸ§ª Connection test successful! Your Kisan Portal Agent is ready.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Kisan Portal Alerts Agent â€” Indian agriculture news & event blog")
